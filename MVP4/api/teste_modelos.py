...
# Parâmetros    
url_dados = "database/manutencao_preventiva3.csv"
...
def test_modelo_svm():
    ...
    acuracia_svm_minmax, recall_svm_minmax, precisao_svm_minmax, f1_svm_minmax = avaliador.avaliar(modelo_svm, X_minmax, Y)
    
    # Obtendo as métricas do SVM para dados com padronização StandardScaler
    acuracia_svm_standard, recall_svm_standard, precisao_svm_standard, f1_svm_standard = avaliador.avaliar(modelo_svm, X_standard, Y)
    
    # Testando as métricas do SVM com normalização MinMaxScaler
    assert acuracia_svm_minmax >= 0.7
    assert recall_svm_minmax >= 0.7
    assert precisao_svm_minmax >= 0.5
    assert f1_svm_minmax >= 0.5
    
    # Testando as métricas do SVM com padronização StandardScaler
    assert acuracia_svm_standard >= 0.7
    assert recall_svm_standard >= 0.7
    assert precisao_svm_standard >= 0.5
    assert f1_svm_standard >= 0.5
